...
from utils.vis_utils import vis_hpe_results

# Multi-scale Multi-stage testing for LIP dataset
def multi_image_testing_on_lip_dataset(net, \
                                       im_root_dir, \
                                       im_name_list, \
                                       transform=None, \
                                       stride=4, \
    ...
    print('LIP Testing with flipping multi-scale: {0} scales'.format(len(scale_multiplier)))
    ...
    for ii in range(0, num_of_im):
        ...
        pose = single_image_testing_on_lip_dataset(net, \
                                                   im, \
                                                   transform=transform, \
                                                   stride=stride, \
                                                   crop_size=crop_size, \
        ...
        print('Testing for LIP dataset: [{0}/{1}], name: {2}, cur time: {3:.4f}, avg time: {4:.4f}'.format(ii + 1, num_of_im, im_name, (end_time - start_time), total_time / (ii + 1)))

    return pose_list 

def single_image_testing_on_lip_dataset(net, \
                                        im, \
                                        transform=None, \
                                        stride=4, \
                                        crop_size=256, \
...
def save_hpe_results_to_lip_format(im_name_list, pose_list, save_path='exps/preds/csv_results/pred_keypoints_lip.csv'):
